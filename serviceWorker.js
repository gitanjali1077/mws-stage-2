...
const cache = 'restaurant-reviews-v1'; // staticCacheName
...
// after registration go for installation
...
self.addEventListener('fetch', event => {
  // event has request parameter and request has url, method, headers etc components 
  event.respondWith(
...
// setup a new  event listener for activation
    ...
    caches.keys().then(cacheNames => {
      ...
      return Promise.all(
        ...
        cacheNames.filter(cacheName => {
          ...
          return cacheName.startsWith('restaurant-reviews-') && cacheName != cache;
